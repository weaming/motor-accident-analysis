...
p('美国国家公路交通安全管理局（NHTSA）致命事故报告系统（Fatality Analysis Reporting System），收录全美所有涉及人员死亡的交通事故记录。')
...
h3('1.3 孟加拉多源交通事故集（新增：时间趋势+碰撞类型）')
...
p()
p('**要点**：')
p('- **高速公路**严重事故率最高（49%），乡村道路最低（22%）')
p('  - ⚠️ 孟加拉的 Highway 并非中国式封闭高速公路，而是相当于国道/省道——限速 40–80 km/h，')
p('    无隔离设施，行人畜力车混行，管理松散')
p('  - 高速路严重率高的主要驱动力是**酒驾率差异**（高速26%，乡村仅13%）和**更高的基础限速**（77 vs 55 km/h）')
p('  - 乡村道路的超速率（84%）和平均超速幅度（+23 km/h）均远高于高速（54%，+8 km/h），')
p('    但因基础速度低，碰撞动能小于高速公路。此结论不宜直接迁移到中国封闭式高速公路场景')
...
p('- 50–59 岁年龄段在摩托车致命事故中占比最高（23%），与普通车辆不同——但此分析缺乏"暴露量"（该年龄段骑手总数）对比，占比高也可能因该年龄段骑手基数大（哈雷等重型巡航车骑手群体集中在 50-60 岁），而非该群体个体风险更高')
p('- 约 53% 的摩托车致命事故发生在乡村道路')

# ── 6. 排量与车型 ──
h2('六、关于排量与车型的讨论')
p('**数据限制**：所有数据集中均不包含摩托车排量（cc）和车型分类信息，是当前分析的局限。')
p()
...
p(f'- **漏报率（False Negative Rate）**：{漏报率_pct:.1f}%（= 漏报 {cm[1, 0]} 例 / 实际严重共 {cm[1, 0] + cm[1, 1]} 例）')
p()
p('**贡献度分析**（基于全模型所有特征）：')
...
p()
p('聊天和吸烟类特征的高贡献度部分源于**代理效应**——它们在模型中充当了骑手"风险类型"的标签，而非完全代表分心行为本身的因果影响。聚类分析见下节。')
...
p()
p('**20 岁以下头盔佩戴者异常**：')
p('图中 20 岁以下佩戴头盔者严重事故率（63.5%）反而高于未佩戴者（30.0%）——该组中佩戴头盔者的酒驾率是未佩戴者的两倍（31% vs 15%），酒驾是最关键的风险因素，')
p('年轻骑手饮酒时也佩戴头盔（法律要求或自我防护），造成头盔"无效"的假象。30 岁以上组中此关系反转，戴头盔者酒驾率更低，保护效果明显。')
p()
p('**干燥路面事故率更高**：')
p('干燥天气下骑手速度更快（均值 84 vs 82 km/h）、酒驾率更高（19% vs 13%），**风险补偿效应**——感知安全时行为更激进，事故后果更严重。')
p('湿滑天气骑手主动降低速度、减少饮酒，事故时撞击速度较低。')
p()
p('**聊天与吸烟的风险聚类**：')
p('聊天和吸烟中等强关联（Cramér V=0.338），两者将骑手分为三个群体：')
p()
...
h2('九、时间趋势与碰撞类型分析')
p()
p('> ⚠️ **数据说明**：本节分析基于 1.3 节介绍的孟加拉多源交通事故集（2007–2021，47,681 条），与 1.1 节核心数据集（15,102 条，含严重程度但无年份字段）来源不同。两个数据集时间跨度、字段结构不完全一致，趋势解读时请注意区分。')
img('trend')
p()
p('**年度趋势要点**：')
...
p('- 摩托车相撞的死亡率为 26.7%，两轮碰撞后果同样严重')
p('- 公交车-摩托车碰撞死亡率最低（12.5%），主要因为公交车主要在拥堵、限速严格的城区道路（City Road）运行，基础车速低；而非公交车本身"更安全"。相比之下，卡车和皮卡大量行驶在不设防的国道上，基础车速高，碰撞动能更大')
p()
...
p()
p(
    '**在一线及核心二线城市**，禁限摩政策导致燃油摩托车在通勤场景中"被迫隐形"，'
    '实际承担大规模中短途通勤和外卖配送的是**电动自行车**（新国标电自，保有量数亿辆）。'
)
p()
p(
    '**但放眼全国**，摩托车仍是不可忽视的交通工具。'
    '截至 2026 年 6 月，中国摩托车总保有量约 **1 亿辆**，'
    '2025 年产销分别达 2210.9 万辆和 2196.7 万辆，仅次于印度位居全球第二。'
    '在三四线城市、乡镇和农村，125–150cc 燃油摩托车依然是家庭不可或缺的生产力与代步工具。'
)
p()
p(
    '同时，中国摩托车市场正在发生**角色分化**：大城市摩托转向休闲娱乐（250cc+ 大排量年销 95 万辆），'
    '而农村和出口市场（年出口 1336 万辆）仍以通勤代步为主。'
    '中国生产的摩托车一半以上出口到东南亚、非洲、南美，'
    '在那些国家扮演着孟加拉数据集中类似的交通工具角色。'
)
p()
p('| 维度 | 一线城市场景 | 全国/三四线/农村 | 电动自行车（通勤补充） |')
p('|------|------------|-----------------|---------------------|')
p('| **车辆类型** | 250cc+ 大排量休闲摩托 | 125–150cc 通勤摩托 | 新国标电自（25–60 km/h） |')
p('| **保有量** | 百万级（集中在大城市摩托车俱乐部） | **1 亿辆**（全国总保有） | 数亿辆 |')
p('| **速度** | 80–200+ km/h | 60–100 km/h | 25–60 km/h |')
p('| **使用场景** | 社交、跑山、长途摩旅 | 通勤、拉货、务农 | 市内短途代步、外卖配送 |')
p('| **法规** | 需驾照/注册/保险，管理严格 | 需驾照，但农村执法薄弱 | 部分城市要求上牌，多数不要求驾照 |')
p('| **头盔** | 佩戴率高 | 佩戴率中等 | 佩戴率低（近年立法后提升） |')
...
p()
p(
    '1. **三四线/农村（125–150cc 通勤摩托）** → 与孟加拉场景最为接近，行为风险排序'
    '（酒驾 > 超速 > 无证）和道路环境分析可直接参考，是最主要的迁移受众。'
)
p(
    '2. **一线城市（250cc+ 休闲摩托）** → 骑行场景差异大（摩旅/跑山 vs 通勤），'
    '但酒驾、超速、分心等基本风险因素依然适用，绝对速度更高意味着致死率可能高于报告数值。'
)
p(
    '3. **电动自行车** → 行为风险定性类似，但绝对速度和致死率需下调预期，'
    '且需补充电池热失控等中国特有安全隐患。'
)
...
p(
    ...
    '此结论不宜直接迁移到中国严格管控的封闭式高速公路场景'
)
p('   Village Road → 县道/乡村路；City Road → 城市道路')
p('2. **分层选取参考维度**：')
p(
    '   - 对于 **三四线/农村交通**（125–150cc 通勤摩托）→ 孟加拉数据的风险排序和环境分析可直接参考，是最主要适用场景'
)
p(
    '   - 对于 **一线城市休闲摩托**（250cc+）→ 参考 FARS 中年龄段、超速占比等特征，'
    '并注意出行目的（摩旅/社交 vs 通勤）带来的暴露量偏差'
)
p(
    '   - 对于 **电动自行车** → 行为风险定性（酒驾、超速、分心）可迁移，'
    '但致死率和碰撞速度需根据较低速度重新校准'
)
p('3. **特色风险**：① 锂电池热失控起火——中国特有的电动自行车安全隐患，本报告未覆盖；'
    '② 雾霾——中国特有的环境因素')
p()
p(
    '需要中国本地摩托车和电动车事故数据进行专项验证。'
    '孟加拉数据可作为"发展中国家两轮交通安全"的参考，'
